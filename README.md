...
`components/` はビルド不要の独立したデザイン部品集です。92種類の部品、昼／夜のテーマ、スマホ幅プレビュー、HTMLコピーを含みます。入力・写真・位置情報・記憶の保存はデモでは行いません。
...
- `components/ar.css` / `components/ar.js`: 泉・炎などのARナビゲーションのデザイン
- [ARの体験と演出の仕様](components/ar-design.md)
- `components/catalog.json`: 全部品の番号・名称・状態
- [詳しい使い方と部品一覧](components/README.md)

コピーしたHTMLには `components.css` と `extended.css`（AR部品は `ar.css` も）が必要です。アプリへの組み込み時は操作処理を接続し、フォームのIDやラジオボタンのnameを重複させないでください。夜の配色は親要素の `data-theme="night"` で指定できます。フォントはGoogle Fontsから読み込み、取得できない場合はシステムフォントを使います。
