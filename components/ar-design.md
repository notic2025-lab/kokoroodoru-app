...
「気配をたどる」体験を、会場の一日の変化へ広げる。陽の下では手触りを感じる道しるべ。日が落ちると、同じ輪郭から光がほどける。場所の意味と形は保ち、明るさに応じて素材を変える。

ARを、場所に記号を貼るためだけに使わない。人が集う、休む、音を聴く、食べる、街へ入る、記憶を残す。それぞれの場所で起きる営みを、風景に宿る「気配」として可視化する。

現実の景色を主役にしたまま、入口から出会いまでの視線を設計する。昼の道しるべと夜の灯り、場所の気配、到着後の記憶を、共通する輪郭でつなぐ。目標は「画面をずっと見続ける」ことではなく、「画面を通して現地に関心を持ち、顔を上げる」こと。

## 昼と夜の設計

| 項目 | 昼 / かたち | 夜 / ひかり |
|---|---|---|
| 場所の目印 | 不透明な色面・白い縁・濃い接地影。立体の厚みを示す形 | 細い流れ・粒・波紋。小さな光のにじみ |
| 道の表現 | 深緑＋白＋深緑の帯。方向に沿った切れ目 | 地面を流れる細い光。明暗をつける |
| 案内文 | 紙色の不透明面＋濃い文字 | 暗い緑の面＋明るい文字 |
| 場所名 | 紙色の面と濃い縁取り | 暗い面と淡い光の縁 |
| 背景への依存 | 発光・透過に頼らず、明るい路面と木陰で輪郭を残す | 発光を使うが、文字の可読性は独立した面で守る |
| 到着 | 形を残し、道の帯と距離を退かせる | 光を弱め、経路と距離を退かせる |

6種類すべてに昼用のSVG形状を追加。色替えだけではなく、面・縁取り・接地表現が異なる。昼の水は青緑、熾火は赤土と琥珀、共鳴は墨紫、湯気は黄土、入口は深緑、記憶は緑と琥珀。名称とシルエットを共通にし、色だけに判断を任せない。

## 時刻による選択

- 初期選択は **自動**。端末の時計を `Asia/Tokyo` に換算する。閲覧端末のタイムゾーンが海外でも、会場の日本時間で判定。
- **06:00以上18:00未満は昼、18:00以上翌06:00未満は夜**。会場向けの暫定時間帯であり、日の出・日の入りの計算ではない。
- 15秒ごと、画面が前面に戻ったとき、フォーカスが戻ったときに再確認。時刻境界から最大15秒程度で反映。
- 昼／夜を手動選択したら、その画面を開いている間は固定。時計更新で上書きしない。自動を選び直すと現在時刻へ戻る。保存はしない。
- 「時刻の切替を試す」では任意の日本時間を入力し、判定を確認できる。確認中は時計の自動更新を止め、画面に「確認用の時刻」と表示。「現在時刻に戻す」で解除。
- 同じ場面・同じ場所のまま昼夜を切り替える。到着済み・残した灯り・測位不良などの状態をリセットしない。
- 部品集のAR切替と埋め込み体験は同期する。親子メッセージは同一オリジンと対象windowを確認。全体の昼／夜ボタンでもARの表示を手動切替できる。
- 「背景を明るくして見比べる」は視認性の比較。カメラの明るさを自動測定する機能ではない。

時刻で判断できるのは初期表示まで。木陰・照明・曇天は時計では判断できないため、実装時も手動切替を残す。背景写真の昼夜はフィルターによる仮表現で、現地の昼夜の実写検証ではない。屋外での見やすさは実機で確認する。
...
- 色：夜の水・波・境界は白に近い石灰色、火は淡い琥珀。昼は上表の不透明な素材色。形と名前を共通にし、運動方向と高さも手がかりにする。
- 昼は不透明な面と縁取り、夜は細い発光を使う。背景フィルターとUI・空間素材を同時に切り替える。環境光推定ではない。
...
73 景色を遮らないヘッダー / 74 光の道 / 75 場所の名前 / 76 必要なときに開く地図 / 77 画面外の気配 / 78 泉 / 79 熾火 / 80 共鳴 / 81 湯気 / 82 境界 / 83 残り灯 / 84 次の一手の案内 / 85 到着後の案内 / 86 位置の確かさ / 87 測位不良 / 88 景色との位置合わせ / 89 接地表現 / 90 昼夜と見やすさの調整 / 91 実景に戻す選択 / 92 自分の灯り。
...
- `experience/experience.js`：場面と昼夜の切替。センサーや永続保存は使わない。部品集との表示同期は同一オリジンのpostMessage。
- `lighting.js`：日本時間への換算、自動・手動の昼夜判定。
- `tests/lighting.test.cjs`：時刻境界、日本時間換算、手動優先、入力の検証。`node --test components/tests/lighting.test.cjs` で実行。
...
## Edition 06.2 — 夜の光を昼にも

昼の不透明な図形と立体物は使用せず、夜と同じ細い光の素材・動きを表示する。昼だけ光の線幅を約 2.2 SVG units に調整し、1px 以下の薄い暗色シャドウで明るい背景との境界を支える。広い発光面や太い縁取りは追加しない。案内の文字は従来の不透明な面で可読性を確保。夜・時刻判定は維持。背景を明るくする比較機能で確認できる。実機の屋外視認性は別途検証が必要。
